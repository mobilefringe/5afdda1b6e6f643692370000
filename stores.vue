    ...
    define(["Vue", "vuex", "vue-select"], function(Vue, Vuex, VueSelect) {
        ...
        return Vue.component("stores-component", {
            ...
            computed: {
                ...
                dropDownCats() {
                    var cats_with_stores = _.filter(this.processedCategories, function(o) {return o.store_ids !== null && o.store_ids.length > 0 });
                    var cats = _.map(cats_with_stores, 'name');
                    cats.unshift('All');
                    return cats;
                },
                // storesList() {
            ...
            methods: {
                ...
                filteredByCategory (cat_id) {
                    ...
                    if(category_id == "All"){
                        this.sortByStores = true;
                        this.allStores;
                    } else if( category_id === "Dine") {
                        //filter by FC Restaurant & FS Restaurant
                        var fc_restaurant_id = 5886;
                        var fs_restaurant_id = 5882;
                        this.sortByStores = false;
                        var find = this.findCategoryById;
                        var filtered = _.filter(this.processedStores, function(o) {return _.includes(o.categories, _.toNumber(fc_restaurant_id)) || _.includes(o.categories, _.toNumber(fs_restaurant_id)); });
                        _.forEach(filtered, function(value, i) {
                            if(_.includes(value.categories, _.toNumber(fc_restaurant_id))){
                                value.currentCategory = find(fc_restaurant_id).name;
                            }
                            else if (_.includes(value.categories, _.toNumber(fs_restaurant_id))){
                                value.currentCategory = find(fs_restaurant_id).name;
                            }
